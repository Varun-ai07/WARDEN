...
function json(res: any, s: number, d: any, headers: Record<string, string> = {}) { res.writeHead(s, { "Content-Type": "application/json", ...headers }); res.end(JSON.stringify(d)); }
...
Return JSON: {"action":"RENEW|SWITCH|DECLINE","target_plan":"plan_id or null","policy_rule_reference":"rule_id","reasoning":"clear explanation citing the specific rule and numbers","confidence":0.0-1.0}`;
      ...
      if (parsed.action) return { subscription_id: sub.id, action: parsed.action, target_plan: parsed.target_plan || null, policy_rule_reference: parsed.policy_rule_reference || policy.rules[0]?.rule_id || "policy_default", reasoning: parsed.reasoning || "AI analysis complete.", confidence: typeof parsed.confidence === "number" ? Math.min(1, Math.max(0, parsed.confidence)) : 0.8 };
...
function fallbackDecide(sub: any, policy: any, portfolioTotal: number): any {
  ...
  return { subscription_id: sub.id, action: "RENEW", target_plan: null, policy_rule_reference: cap?.rule_id || "policy_default", reasoning: `${sub.merchant_name} remains within the policy. Current portfolio: $${(portfolioTotal / 100).toFixed(2)}/month.`, confidence: 0.9 };
}
